...
class ExMain(QWidget):
    def __init__(self):
        ...
        rospy.Subscriber('/lidar_topic',
                         self.lidar_callback)  # change /lidar_topic to whatever the topic is called
    ...
    def doYourAlgorithm(self, points):
        ...
        for i in range(1, clusterCnt):
            ...
            if (abs(x_size) <= carLength + 1) and (
                    abs(y_size) <= carHeight + 1):  # 차량 길이 비교할 때 마이너스로 비교하면 X / 따라서 절대값으로 비교
                tempobjPos[0] = x
                tempobjPos[1] = y
